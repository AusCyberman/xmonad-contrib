{-# LANGUAGE FlexibleContexts, PatternGuards, TupleSections, TypeApplications #-}
...
module XMonad.Hooks.DynamicLog (
    ...
    -- * Dynamic Icons
    dynamicLogIconsWithPP, dynamicLogIconsConvert, appIcon, IconSet,
    IconConfig(..), Icon(..),
...
workspaceFilter iconConfig = do
    ws <- gets (S.workspaces . windowset)
    icons <- M.fromList . (maybeToList =<<) <$> mapM (getIcons (iconConfigIcons iconConfig)) ws
    pure $ map (\x -> x {S.tag = ppSection ppCurrent iconCurrent icons (S.tag x) } )
  where
    ppSection pF f icons = pF (iconConfigPP iconConfig) . concatIcons f . iconLookup icons
    iconLookup icons x = M.findWithDefault [baseIconSet x] x icons
    concatIcons f y
